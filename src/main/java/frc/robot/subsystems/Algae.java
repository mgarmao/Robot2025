...
public class Algae extends SubsystemBase {
    ...
    public Algae() {
        CurrentLimitsConfigs configs1 = new CurrentLimitsConfigs().withStatorCurrentLimit(Constants.CurrentLimits.algaeRotator).withSupplyCurrentLimit(Constants.CurrentLimits.algaeRotator).withStatorCurrentLimitEnable(true).withSupplyCurrentLimitEnable(true);
        rotatorMotor.getConfigurator().apply(new TalonFXConfiguration().withCurrentLimits(configs1));

        CurrentLimitsConfigs configs2 = new CurrentLimitsConfigs().withStatorCurrentLimit(Constants.CurrentLimits.algaeIntakeWheels).withSupplyCurrentLimit(Constants.CurrentLimits.algaeIntakeWheels).withStatorCurrentLimitEnable(true).withSupplyCurrentLimitEnable(true);
